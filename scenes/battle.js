function setBattle(worldState) {
  ...
  const content = box.add([
    text("MIRA is ready to battle!", { size: 42 }),
    color(10, 10, 10),
    pos(20, 20),
  ])
  ...
  onUpdate(() => {
    ...
    if (enemyMonHealthBar.width < 0 && !enemyMon.fainted) {
      ...
      setTimeout(() => {
        content.text = "MIRA won the battle!"
      }, 1000)
      setTimeout(() => {
    ...
    if (playerMonHealthBar.width < 0 && !playerMon.fainted) {
      ...
      content.text = "MIRA fainted!"
      playerMon.fainted = true
      setTimeout(() => {
        content.text = "You rush to get MIRA healed!"
      }, 1000)
      setTimeout(() => {
